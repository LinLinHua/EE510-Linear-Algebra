...
% create bigger frame and center the picture
Mpad = zeros(rows + RowPad, cols + ColPad, 3);
row_offset = ceil(RowPad / 2);
col_offset = ceil(ColPad / 2);

% paste the original image to new frame
Mpad(row_offset+1:row_offset+rows, col_offset+1:col_offset+cols, :) = M;

% reset the new frame image
[m_rows, m_cols, channels] = size(Mpad);
cols_pad = m_cols;
rows_pad = m_rows;

% rotated image
Mrotate = zeros(m_rows, m_cols, 3);

% midpoint
midx = ceil((m_cols + 1) / 2);
midy = ceil((m_rows+1) / 2);


%% Step 1: Floor/Ceil Pull Technique
% Loop through each pixel in the output image
for x_new = 1:cols_pad
    for y_new = 1:rows_pad
        % calculate the original(old) image location
        original_cor = Rinv * ([x_new - midx; y_new - midy]) + [midx; midy];
        x_old = original_cor(1);
        y_old = original_cor(2);

        % Apply floor/ceil rounding
        x_floor = floor(x_old)-20;
        y_floor = floor(y_old)-50;
        x_ceil = ceil(x_old)-20;
        y_ceil = ceil(y_old)-50;

        % Check the boundary and floor/ceil it
        ...
        elseif x_ceil >= 1 && x_ceil <= cols && y_ceil >= 1 && y_ceil <= rows
...
imwrite(Mrotate, 'RotatedImage2.jpg', 'jpg');
imshow(Mrotate);
...
%newbwimage = im2uint8(B);
imwrite(Mrotate, 'EnhancedRotatedImage2.jpg', 'jpg');
imshow(Mrotate);
